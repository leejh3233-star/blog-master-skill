...
## 14. 실제 SmartEditor 편집·모바일 UX 연동

구조화 JSON을 실제 네이버 SmartEditor 또는 브라우저 자동화에 넣어 게시할 때는 `knowledge/12-mobile-publishing-ux.md`를 함께 적용한다.

추가 적용:
- 주요 소제목은 필요할 때 SmartEditor의 왼쪽 세로선 인용구형 디자인을 사용할 수 있다.
- 인용구형 소제목을 실제 타인의 발언처럼 오해시키지 않는다.
- 본문은 모바일에서 1~3개의 짧은 문장 중심으로 유지한다.
- 본문·소제목·포인트·주의 색상을 제한된 팔레트로 통일한다.
- 글자색·굵기·인용구는 가독성 장치이며 SEO·AI 인용의 보장 조건으로 취급하지 않는다.
- SmartEditor 삽입 후 링크가 유지되는지 실제 클릭으로 다시 확인한다.
- 모바일 미리보기가 가능하면 MOBILE_READER_QA를 수행한다.

구조화 출력이 편리하더라도 FINAL_ARTICLE의 팩트·조건·링크를 편집 단계에서 임의로 바꾸지 않는다.
